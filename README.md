...
```bash
git clone https://github.com/bkostic006-dev/TaskManager.git
cd TaskManager
docker compose up
```
...
**Three rate limits, not one.** `/auth/login` and `/auth/signup` allow 10 a minute; `GET /tasks` allows 240; everything else 120. A single app-wide number cannot serve both ends: tuned to make credential guessing expensive it answers `429` to a dashboard changing filters, and tuned for browsing it is worthless against a login attacker. The auth number is 10 rather than 5 because the throttler runs _before_ the validation pipe, so a rejected field spends the allowance exactly as a wrong password does — and the signup form is `noValidate`, so a new user who trips the password minimum, then the email format, then finds the address taken would be told "too many requests" while filling in a form for the first time.

**The task list cache is keyed on the user, and that is a tenancy control rather than a performance note.** `GET /tasks?page=1&pageSize=8` is byte-identical for every user of this system — the identity lives in the `Authorization` header, which NestJS's default URL-based cache key never sees. Dropped in as it comes, the first user to load page 1 fills the cache and the second is served _their_ rows, undoing every other tenancy control in the codebase. So `userId` is the first component of the key, taken from the verified token. Two users issuing the identical query string getting different rows is asserted by a test, and that test was checked by removing `userId` from the key and confirming it fails.

**The `design/` folder is committed on purpose, and it is not product code.** It holds the design source this UI was built from — `tokens.ts` (the palette, spacing, radii and type scale that became `apps/web/src/theme.ts`), `components.md` (which Mantine component builds which element), and static HTML mockups of the login and dashboard screens. It was produced with Claude, as an exploratory pass to settle the visual decisions before any component was written, and it is included so a reviewer can see what the interface was built against rather than having to infer it. **Nothing in `apps/` imports from it** — it is a reference that removed decisions, not a specification the code is bound to, and the UI departs from it in places where behaviour won a disagreement with the mockup (a completed row crosses its numeral off rather than replacing it with a check glyph, for instance, because the checkbox beside it already carries a tick).

**Pinned to Prisma 6, Next 15 and Mantine 8 rather than the latest majors.** Prisma 7, Next 16 and Mantine 9 are all current. The brief states library versions are unspecified and nothing requires latest majors, and each upgrade costs something real here: Prisma 7 changes the ESM and driver-adapter handling that both Dockerfiles already work around, Next 16 would invalidate the browser QA pass that validated Next 15, and Mantine 9 would break the design tokens this UI was built against. A major upgrade immediately before submission risks the one thing the brief does demand — that `docker compose up` works for a reviewer. NestJS is on 11.1.29 against a current 11.2.1 — same major, one minor behind, and the lockfile is what pins it, so a fresh clone gets the same build this was tested on. v12 is an alpha whose companion packages still cap at `^11`.
